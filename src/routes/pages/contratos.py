...
async def list_contrato(
    ...
    items, total_pages, page, per_page, total_count = await service.get_list_paginated()
    ...
    context = {
        "user": user,
        "page": page_info,
        "total_count": total_count,  # Passa o valor real para o card
        "current_page": page,
...
async def list_contrato_items(request: Request, page: int = 1, db=Depends(get_db)):
    ...
    return render_chunk(request, template, context, block="table")
